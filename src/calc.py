...
from dry.mathutils import GRAV_ACC, rounddown, roundup
from dry.measurements import (nm, gram, kg, kw, meter, mm, rpm, to_kw, to_mm,
                              to_rpm)
from dry.numcompar import is_lesseq
...
STEEL_SPACERS = {
    mm(3): Spacer(material=STEEL, weight=gram(40.76), s=mm(3), designation=""),
    mm(6): Spacer(material=STEEL, weight=gram(81.51), s=mm(6), designation="")
}
...
PLASTIC_SHEET_LENGTH = meter(3.0)

PLATES_IN_SHEET = rounddown(PLASTIC_SHEET_WIDTH / PLATE_HEIGHT)

GAP_ACCURACY = mm(0.05)

# Угол наклона решетки.
TILT_ANGLE = radians(50)
...
class StepScreen:
    def __init__(self, inpdata: InputData, adderror: AddMsgL10n) -> None:
        ...
        if inpdata.plate_spacer.spacer == PLASTIC:
            ...
            self.spacer = PLASTIC_SPACERS[self.nominal_gap]
        else:
            self.optimal_gap = inpdata.gap + STEEL_GAP_MIN
            self.spacer = STEEL_SPACERS[self.nominal_gap]
        ...
        self.gap = (self.step - self.steel_fixed.s - self.steel_moving.s) / 2
        self.bottom_spacer_s = mm(rounddown(to_mm(self.gap - STEEL_GAP_MIN)))
        ...
        self.raidus = (self.axe_x**2 + self.axe_y**2)**0.5
    ...
    def calc_single_plastic(self) -> float:
        ...
        assert is_lesseq(xgap, PLASTIC_GAP_MAX)
        return plastic

    def calc_double_plastic(self) -> tuple[float, float]:
        space = mm(rounddown(to_mm(self.optimal_step - 2 * PLASTIC_GAP_MIN)))
        plastics = DOUBLE_PLASTIC_PLATES[space]
        xgap = (self.optimal_step - plastics[0] - plastics[1]) / 2
        assert is_lesseq(xgap, PLASTIC_GAP_MAX)
        return plastics

    def calc_moving_count_and_step(self, fixed_s: float) -> tuple[int, float]:
        movcount = rounddown((self.inner_width + fixed_s) / self.optimal_step)
        step = (self.inner_width + fixed_s) / movcount
        return movcount, step

    def calc_side_gap(self, moving_s: float) -> float:
        return (self.inner_width - moving_s - self.step * self.fixed_count) / 2

    def calc_plastic_sheet(self, plate_count: int) -> int:
        return roundup(plate_count / PLATES_IN_SHEET)
    ...
    def calc_mintorque(self) -> float:
        ...
        return self.weight.moving * unaccounted_load * GRAV_ACC * LEVER_ARM

    def calc_drive(self) -> Drive | None:
        drives = DRIVE_UNITS_05XX if self.ws <= 5 else DRIVE_UNITS
        for i in drives:
            if i.torque >= self.mintorque:
                return i
        return None
...
def create_result(scr: StepScreen, addline: AddMsgL10n) -> None:
    addline(Output.RSK_GAP_DEPTH, scr.ws, scr.hs, to_mm(scr.gap),
            to_mm(scr.steel_fixed.s), to_mm(scr.steel_moving.s),
            to_mm(scr.depth))
    if scr.drive:
        addline(Output.WEIGHT, scr.weight.full)
        addline(Output.DRIVE, scr.drive.designation, to_kw(scr.drive.power),
                to_rpm(scr.drive.speed), scr.drive.torque)
    else:
        addline(Output.DRIVELESS_WEIGHT, scr.weight.full)
        addline(Output.DRIVE_UNDEF)
    addline("")
    addline(Output.EXT_WIDTH, to_mm(scr.outer_width))
    addline(Output.INT_WIDTH, to_mm(scr.inner_width))
    addline(Output.DROP_WIDTH, to_mm(scr.dropwidth))
    addline(Output.FULL_DROP_HEIGHT, to_mm(scr.full_dropheight))
    addline(Output.DROP_HEIGHT, to_mm(scr.dropheight))
    addline(Output.SCR_HEIGHT, to_mm(scr.height))
    addline(Output.SCR_LENGTH, to_mm(scr.length))
    addline(Output.HORIZ_LENGTH, to_mm(scr.horiz_lenght))
    addline(Output.AXE_X, to_mm(scr.axe_x))
    addline(Output.RADIUS, to_mm(scr.raidus))
    addline("")
    addline(Output.FOR_DESIGNER)
    addline("")
    addline(Output.MIN_SIDE_GAP, to_mm(scr.min_side_gap))
    ...
    addline(Output.MOVING_WEIGHT, scr.weight.moving)
    addline(Output.MIN_TORQUE, scr.mintorque)
    addline("")
...
def create_equations(scr: StepScreen, addline: AddMsgL10n) -> None:
    ...
    # в меньшую сторону
    pmb_int_number = rounddown(pmb_float_number)
    # Болты крепления пластин: количество болтов
    if pmb_float_number > pmb_int_number:
    ...
    dpb_max_size = scr.inner_width - 0.045
    ...
    dpc_max_size = scr.inner_width - 0.103
    ...
    addline("\"inner_width\" = {:g}mm  ''Внутрішня ширина решітки",
    ...
    addline("\"main_gap\" = {:g}mm  ''Прозор між пластинами",
            to_mm(scr.gap))

    addline("\"teeth_number\" = {}  ''Кількість зубів пластин (для масиву)",
            scr.all_teeth)
    ...
    addline("\"step\" = {:g}mm  ''Крок між пластинами одного полотна",
            to_mm(scr.step))

    addline("\"number_fixed\" = {}  ''Кількість нерухомих пластин",
            scr.fixed_count)

    addline("\"number_moving\" = {}  ''Кількість рухомих пластин",
            scr.moving_count)

    addline("\"side_gap\" = {:g}mm  "
            "''Зазор між боковиною та крайньою пластиною",
            to_mm(scr.side_steel_gap))

    addline("\"start_fixed\" = {:g}mm  "
            "''Відстань від боковини до середини нерухомої пластини",
            to_mm(scr.fixed_start))

    addline("\"start_moving\" = {:g}mm  "
    ...
    addline("\"pmb_number\" = {}  ''Болти кріплення пластин: кількість болтів",
            pmb_number)
    ...
    addline("\"pmb_start\" = {:g}mm  ''Болти кріплення пластин: "
    ...
    addline("\"dpb_count\" = {}  ''Болти скидання: кількість болтів",
            dpb_count)

    addline("\"dpb_step\" = {:g}mm  ''Болти скидання: крок між болтами",
            to_mm(dpb_step))
...
class WeightSet:
    def __init__(self, scr: StepScreen):
        ...
        self.topcover_fixedbeam = 0.167143 * scr.ws + 0.167143
        self.front_cover00_fixedbeam = 0.191429 * scr.ws + 0.211429
        self.front_cover01_fixedbeam = 0.265714 * scr.ws + 0.155714
        self.fixing_strip = kg(0.35)
        self.terminalbox = kg(5.95)
        self.connectingrod = kg(2.5)
        self.crank = kg(4.82)
        self.bottomflap = 0.105714 * scr.ws - 0.0842857
        self.plateclip_fixed = 0.428571 * scr.ws - 0.371429
        self.plateclip_moving = 0.435714 * scr.ws - 0.334286
        self.bottonrake = 0.175714 * scr.ws - 0.164286
        self.bottom_framebeam = 0.405714 * scr.ws - 0.064286
        self.middle_framebeam = 0.534286 * scr.ws - 0.365714
        self.top_framebeam = 0.662857 * scr.ws + 0.0728571
        self.platebeam_fixed = 0.644286 * scr.ws - 0.505714
        self.platebeam_moving = 0.648571 * scr.ws + 3.34857
        self.sidewall = 2.56889 * scr.hs + 37.32
        self.parallelogram_beam = 0.785556 * scr.hs + 6.39
        self.moving_lengthwise_beam = 1.15667 * scr.hs + 4.45
        self.sidecover = 8.67532 * scr.dropheight + 12.498
        self.bottom_spacer = 110 * scr.bottom_spacer_s - 0.01
        ...
        if scr.plastic_fixed:
            self.steel_fixed = \
                75 * scr.steel_fixed.s * scr.steel_fixed.teeth \
                + 215 * scr.steel_fixed.s
            self.plastic_fixed = \
                8.88889 * scr.plastic_fixed.s * scr.plastic_fixed.teeth \
                + 14.4444 * scr.plastic_fixed.s
            fixpl = self.steel_fixed + self.plastic_fixed
        else:
            self.steel_fixed = \
                73.3333 * scr.steel_fixed.s * scr.steel_fixed.teeth \
                + 653.333 * scr.steel_fixed.s
            self.plastic_fixed = None
            fixpl = self.steel_fixed
        self.fixed_plates = \
            scr.fixed_count * fixpl + scr.spacer_count * scr.spacer.weight \
            + self.bottom_spacer * scr.bottom_spacer_count

        self.plastic_moving: float | None
        if scr.plastic_moving:
            self.steel_moving = \
                73.3333 * scr.steel_moving.s * scr.steel_moving.teeth \
                + 230 * scr.steel_moving.s
            self.plastic_moving = \
                6.66667 * scr.plastic_moving.s * scr.plastic_moving.teeth \
                + 40 * scr.plastic_moving.s
